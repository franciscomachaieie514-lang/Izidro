...
export default function Home(){
 ...
 useEffect(()=>{load();const p=new URLSearchParams(location.search);if(p.get('deriv_connected'))setMessage('Conta Deriv conectada com sucesso.');if(p.get('auth_error'))setError(p.get('auth_error')||'Erro de autenticação.')},[]);
...
function Brand(){
 ...
 useEffect(()=>{
  const normalize=()=>{const value=localStorage.getItem('izitrader_lang');setLang(value==='en'||value==='es'||value==='pt'?value:'pt')};
  normalize();
