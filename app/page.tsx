...
export default function Home(){
 ...
 if(auth.loading)return <main className="page"><div className="shell"><div className="page-head"><Brand lang={lang}/><LanguagePicker lang={lang} onChange={changeLanguage}/></div><div className="card muted">{t.loading}</div></div></main>;
 if(auth.user&&!auth.derivConnected)return <main className="page"><div className="shell"><div className="page-head"><Brand lang={lang}/><div className="user"><strong>{auth.user.name}</strong><small>{auth.user.email}</small></div><LanguagePicker lang={lang} onChange={changeLanguage}/></div><div className="card"><h1 className="title">{t.account}</h1><p className="muted">{t.accountText}</p><div className="connect card"><h2>{t.connect}</h2><p className="muted">{t.connectText}</p><a className="btn primary" style={{display:'block',textAlign:'center',textDecoration:'none'}} href="/api/auth/login">{t.continueDeriv}</a></div>{error&&<div className="error">{error}</div>}{message&&<div className="success">{message}</div>}<button className="btn secondary" onClick={logout}>{t.logout}</button><RiskWarning lang={lang}/></div></div></main>;
 return <main className="page"><div className="shell"><div className="page-head"><Brand lang={lang}/><LanguagePicker lang={lang} onChange={changeLanguage}/></div><div className="card"><div className="tabs"><button className={`tab ${mode==='register'?'active':''}`} onClick={()=>{setMode('register');setError('')}}>{t.createTab}</button><button className={`tab ${mode==='login'?'active':''}`} onClick={()=>{setMode('login');setError('')}}>{t.loginTab}</button></div><h1 className="title">{mode==='register'?t.createTitle:t.loginTitle}</h1><p className="muted">{mode==='register'?t.createText:t.loginText}</p><form onSubmit={submit}>{mode==='register'&&<><label className="label">{t.name}</label><input className="input" value={name} onChange={e=>setName(e.target.value)} autoComplete="name" required/></>}<label className="label">{t.email}</label><input className="input" type="email" value={email} onChange={e=>setEmail(e.target.value)} autoComplete="email" required/><label className="label">{t.password}</label><input className="input" type="password" value={password} onChange={e=>setPassword(e.target.value)} autoComplete={mode==='register'?'new-password':'current-password'} required/>{mode==='register'&&<><label className="label">{t.confirm}</label><input className="input" type="password" value={confirm} onChange={e=>setConfirm(e.target.value)} autoComplete="new-password" required/></>}<button className="btn primary" disabled={busy}>{busy?t.wait:mode==='register'?t.create:t.login}</button></form>{error&&<div className="error">{error}</div>}{message&&<div className="success">{message}</div>}<div className="foot">{t.foot}</div><RiskWarning lang={lang}/></div></div></main>;
}

function LanguagePicker({lang,onChange}:{lang:Lang;onChange:(lang:Lang)=>void}){
 const flags=[['en','🇬🇧','English'],['pt','🇵🇹','Português'],['es','🇪🇸','Español']] as const;
 return <div aria-label="Language" style={{display:'flex',flexDirection:'row',alignItems:'center',gap:5,position:'absolute',top:0,right:0}}>{flags.map(([code,flag,name])=><button key={code} type="button" onClick={()=>onChange(code)} aria-label={name} title={name} style={{width:30,height:22,padding:0,border:lang===code?'1px solid #e30613':'1px solid var(--border,#dde2ec)',borderRadius:6,background:lang===code?'rgba(227,6,19,.08)':'var(--field,#eef1f7)',cursor:'pointer',fontSize:14,lineHeight:1}}>{flag}</button>)}</div>
}
...
function Brand({lang}:{lang:Lang}){
 return <div className="brand" style={{display:'flex',alignItems:'flex-start',lineHeight:1}}><div className="logo" style={{marginRight:10,flexShrink:0}}>I</div><div style={{display:'flex',flexDirection:'column',alignItems:'flex-start',justifyContent:'center'}}><div className="brandName" style={{fontSize:28,fontWeight:800,lineHeight:1.05}}>Izi<span>trader</span></div><div style={{marginTop:2,fontSize:10,fontWeight:600,letterSpacing:'.02em',color:'#7c88a3',whiteSpace:'nowrap'}}>{SLOGANS[lang]}</div></div></div>
}
