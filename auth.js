(async function(){
  ...
  function showApp(connectDeriv){if(authPage)authPage.classList.add('hidden');if(appPage)appPage.classList.remove('hidden');if(connectDeriv)loadDerivWS();}
  function showAuth(){if(window.IziDerivWS&&typeof window.IziDerivWS.disconnect==='function')window.IziDerivWS.disconnect();if(appPage)appPage.classList.add('hidden');if(authPage)authPage.classList.remove('hidden');}
  ...
  try{const response=await fetch('/api/auth/session',{credentials:'same-origin',cache:'no-store'});const session=await response.json();if(session.authenticated)showApp(true);else if(localStorage.getItem('izitrader_demo')==='1')showApp(false);else showAuth();}
