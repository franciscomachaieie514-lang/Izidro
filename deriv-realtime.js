/* Izitrader realtime bridge — no React/CDN dependency.
 * OAuth session -> authenticated OTP WebSocket -> balance + ticks + contracts.
 */
(function(){
  'use strict';
  ...
  let uiSymbol='R_100',symbol='1HZ100V',botName='ProParity',stake=.35;
  let ticks=[],active=false,buying=false,proposal=null,contractId=null,pnl=0,contractSubscription=null;
  ...
  async function connect(){
    ...
    currentBot();currentStake();currentSymbol();
    setStatus(`A ligar à conta ${accountType==='real'?'Real':'Demo'}…`,false);
    try{
      const aj=await getJson('/api/deriv/accounts');
      ...
      if(!account)throw new Error(`Conta ${accountType==='real'?'Real':'Demo'} não encontrada pela API Deriv`);
      const id=accountId(account);if(!id)throw new Error('A API Deriv não devolveu account_id');
      const wj=await getJson('/api/deriv/ws-url?account_id='+encodeURIComponent(id));
      const url=wj&&((wj.data&&wj.data.url)||wj.wsUrl);if(!url)throw new Error('Deriv não devolveu URL WebSocket autenticada');
      if(stopped)return;
