(function(){
  const statusEl=document.querySelector('.status'); const balanceEl=document.querySelector('.value');
  const symbolMap={R_100:'1HZ100V',R_75:'1HZ75V',R_50:'1HZ50V',R_25:'1HZ25V',R_10:'1HZ10V'};
  let ws=null,reconnectTimer=null;
  function setStatus(text,connected){if(statusEl)statusEl.innerHTML=`<span class="dot" style="background:${connected?'#35d492':'#f5a623'}"></span>${text}`;}
  ...
  function showTick(data){const tick=data&&data.tick;if(!tick)return;window.dispatchEvent(new CustomEvent('izitrader:tick',{detail:{quote:Number(tick.quote),epoch:tick.epoch,symbol:tick.symbol}}));}
  ...
  async function connect(){
    try{
      ...
      ws.onerror=()=>setStatus('Erro na ligação Deriv',false); ws.onclose=()=>{setStatus('Deriv desligada — a reconectar…',false);clearTimeout(reconnectTimer);reconnectTimer=setTimeout(connect,3000);};
    }catch(error){setStatus(error.message||'Falha na ligação Deriv',false);clearTimeout(reconnectTimer);reconnectTimer=setTimeout(connect,5000);}
  }
