...
function clearHistory(){const h=getHistory();if(!h)return;h.innerHTML='';const count=$('#opsCount');if(count)count.textContent='0'}
function scrollFix(){const h=getHistory();if(!h)return;clearHistory();h.style.overflowX='hidden';h.style.overflowY='hidden';h.style.display='flex';h.style.flexWrap='nowrap';h.style.gap='5px'}
...
window.addEventListener('izitrader:contract-closed',e=>{sessionPnl+=n((e.detail||{}).profit_loss??(e.detail||{}).profit);clearHistory();checkTargets()});
window.addEventListener('storage',()=>{scrollFix();checkTargets()});
window.addEventListener('izitrader:buy',()=>{clearHistory();setTimeout(clearHistory,0)});
})();