/* React Deriv bridge: one authenticated real-time WebSocket for ticks, balance, P/L and proposal preview. */
(function(){
  ...
  async function boot(){
    ...
    function useDerivBot({accountType='demo',symbol='1HZ100V',botName='ProParity',stake=.35}){
      const ws=useRef(null),req=useRef(1000),ticks=useRef([]),lastProposal=useRef(0),startBalance=useRef(null);
      const [state,setState]=useState({connected:false,authorized:false,tick:null,balance:null,pnl:0,proposal:null,signal:null,error:null});
      ...
      const connect=useCallback(async()=>{try{
        ...
        const accounts=aj.data||aj.accounts||[];const wanted=accountType==='real'?accounts.find(a=>!(a.is_virtual||a.account_type==='demo'||a.type==='demo')):accounts.find(a=>a.is_virtual||a.account_type==='demo'||a.type==='demo');
        const a=wanted||accounts[0];if(!a)throw new Error('Nenhuma conta Deriv disponível');const id=a.account_id||a.accountId||a.id;if(!id)throw new Error('ID da conta Deriv não encontrado');
        ...
        if(!wr.ok||!wj?.data?.url)throw new Error(wj.error||'Não foi possível abrir WebSocket Deriv');if(ws.current)try{ws.current.close()}catch{}
        const socket=new WebSocket(wj.data.url);ws.current=socket;
        socket.onopen=()=>{setState(s=>({...s,connected:true,authorized:true,error:null}));socket.send(JSON.stringify({balance:1,subscribe:1,req_id:++req.current}));socket.send(JSON.stringify({ticks:symbol,subscribe:1,req_id:++req.current}));window.dispatchEvent(new CustomEvent('izitrader:ws-open',{detail:{socket}}));};
        socket.onmessage=e=>{let m;try{m=JSON.parse(e.data)}catch{return};
          ...
          if(m.msg_type==='balance'&&m.balance){
            const current=Number(m.balance.balance);const balance={balance:current,currency:m.balance.currency||'USD',loginid:m.balance.loginid||null};
            if(startBalance.current==null)startBalance.current=current;const pnl=current-startBalance.current;
            setState(s=>({...s,balance,pnl,error:null}));
            const bel=document.getElementById('balance');if(bel)bel.textContent=`${current.toFixed(2)} ${balance.currency}`;
            const pel=document.getElementById('pnl');if(pel){pel.textContent=`${pnl>=0?'+':''}${pnl.toFixed(2)} ${balance.currency}`;pel.classList.toggle('profit',pnl>=0);pel.style.color=pnl<0?'#e24b4a':'';}
            window.dispatchEvent(new CustomEvent('izitrader:balance',{detail:{...balance,pnl}}));
          }
          if(m.msg_type==='tick'&&m.tick){const q=Number(m.tick.quote);ticks.current=[...ticks.current,q].slice(-100);setState(s=>({...s,tick:m.tick,signal:signal(ticks.current,botName)}));window.dispatchEvent(new CustomEvent('izitrader:tick',{detail:{quote:q,quoteRaw:String(m.tick.quote),epoch:m.tick.epoch,symbol:m.tick.symbol}}));}
          if(m.msg_type==='proposal'&&m.proposal){setState(s=>({...s,proposal:m.proposal,error:null}));window.dispatchEvent(new CustomEvent('izitrader:proposal',{detail:m.proposal}));}
        };
        socket.onclose=()=>{setState(s=>({...s,connected:false,authorized:false}));window.dispatchEvent(new CustomEvent('izitrader:ws-close'));};socket.onerror=()=>setState(s=>({...s,error:'Erro no WebSocket Deriv'}));
    ...
    function Bridge(){
      const [bot,setBot]=useState(document.getElementById('selectedBotName')?.textContent?.trim()||'ProParity');const [symbol,setSymbol]=useState('1HZ100V');const [account,setAccount]=useState('demo');const [stake,setStake]=useState(.35);
      ...
      useEffect(()=>{const btn=document.querySelector('.operate-btn');if(!btn)return;const handler=()=>{if(!d.connected){alert('Aguardando ligação à Deriv.');return}if(!d.signal){alert('Aguardando sinal de 65% nos últimos 5 ticks.');return}d.requestProposal();btn.textContent='PROPOSTA SOLICITADA';setTimeout(()=>{btn.textContent='INICIAR ROBÔ'},1200)};btn.addEventListener('click',handler);return()=>btn.removeEventListener('click',handler)},[d.connected,d.signal,d.requestProposal]);
